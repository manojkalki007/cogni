...
import { Plug, CheckCircle, XCircle, AlertTriangle, Loader2, ExternalLink } from "lucide-react";
...
const INTEGRATION_META = {
  salesforce: { icon: "⚡", color: "text-blue-400", description: "Sync contacts, log calls as Tasks, auto-create Leads" },
  hubspot: { icon: "🟠", color: "text-orange-400", description: "Push call data and contact updates to HubSpot CRM" },
  zoho: { icon: "🔵", color: "text-indigo-400", description: "Connect to Zoho CRM for lead and contact sync" },
  google_calendar: { icon: "📅", color: "text-green-400", description: "Check availability and book appointments via Google Calendar" },
  razorpay: { icon: "💳", color: "text-emerald-400", description: "Generate and send payment links during calls" },
  webhook: { icon: "🔗", color: "text-purple-400", description: "Send call events to custom HTTP endpoints" },
};
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
            ...
            <div key={f.key}>
              ...
              {f.type === "textarea" ? (
                <textarea value={config[f.key] || ""} onChange={(e) => setConfig({ ...config, [f.key]: e.target.value })}
                  rows={4} className="w-full bg-gray-800 border border-gray-700 rounded-md px-3 py-2 resize-none font-mono text-xs"
                  placeholder={f.placeholder} />
              ) : (
                <input type={f.type} value={config[f.key] || ""} onChange={(e) => setConfig({ ...config, [f.key]: e.target.value })}
                  className="w-full bg-gray-800 border border-gray-700 rounded-md px-3 py-2" placeholder={f.placeholder} />
              )}
            </div>
          ))}
        </div>
        ...
        {testResult && (
          <div className={`text-sm p-2 rounded ${testResult.status === "ok" ? "bg-green-900/30 text-green-400" : "bg-red-900/30 text-red-400"}`}>
...
function IntegrationCard({ integration, onClick }) {
  ...
  return (
    <Card className="cursor-pointer hover:border-gray-600 transition-colors" onClick={onClick}>
      <CardContent className="p-5">
        ...
        {integration.last_sync_at && (
          <p className="text-[10px] text-gray-600 mt-2">Last sync: {new Date(integration.last_sync_at).toLocaleString()}</p>
        )}
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Badge variant="outline">{connected.length} connected</Badge>
      </div>
